...
#
# Background: the Dockerfile sets PYTHONPATH=/root, so Python imports
# /root/hedweb/_version.py (the committed stub) instead of the file that
# pip writes to site-packages.  We overwrite the stub here, before the
# image is built, so the correct version reaches the running container.
generate_version_file() {
    ...
    if [ -n "${raw_describe}" ]; then
        ...
        commit_node=$(echo "${raw_describe}" | sed 's/.*-g/g/')
        if [ "${distance}" = "0" ]; then
            version="${tag}"
        else
...
setup_web_directory() {
    ...
    # Generate _version.py with real git info so the footer shows the correct
    # version inside Docker.  Without this step the container would import
    # /root/hedweb/_version.py (picked up first via PYTHONPATH=/root), which is
    # only a stub of type annotations with no actual assignments, resulting in
    # web_ver = "unknown" and web_commit = "" in the page footer.
    generate_version_file
}
